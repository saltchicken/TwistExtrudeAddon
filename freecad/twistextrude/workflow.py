...
from freecad.twistextrude.feature import TwistExtrudeFeature
from freecad.twistextrude.feature import ViewProviderTwistExtrude
...
def inject_twist_into_document(profile_obj, path_obj, config: TwistConfig):
    ...
    if active_body:
        feat_obj = App.ActiveDocument.addObject("PartDesign::FeaturePython",
                                                feat_name)
        active_body.addObject(feat_obj)
        active_body.Tip = feat_obj
    else:
        feat_obj = App.ActiveDocument.addObject("Part::FeaturePython",
                                                feat_name)
    ...
    if feat_obj.ViewObject and profile_obj.ViewObject:
        feat_obj.ViewObject.ShapeColor = getattr(profile_obj.ViewObject,
